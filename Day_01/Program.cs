...
namespace Day_01
{
	internal class Program
	{
		...
		private static readonly Matrix3x2 _rotateLeft = Matrix3x2.CreateRotation((float) (Math.PI / 2));
		private static readonly Matrix3x2 _rotateRight = Matrix3x2.CreateRotation((float) (-Math.PI / 2));
		private static List<Vector2> _historicPositions;

		private static Vector2 ParseStep(string step, Vector2 pos)
		{
			if (step.Length < 2)
		...
		private static bool VectorEquals(Vector2 a, Vector2 b)
		{
		...
		private static void Main(string[] args)
		{
			while (true)
			{
				...
				{
					var count = _historicPositions.Count(x => VectorEquals(x, pos));
